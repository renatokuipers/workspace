...
const { resolveFlowScriptPath, resolvePythonExecutable } = require('../utils/pathResolver');
...
class ProcessManagerService extends EventEmitter {
  constructor() {
    ...
    this.scriptPath = resolveFlowScriptPath();
  ...
  async start() {
    ...
    console.log('Full script path:', require('path').resolve(this.scriptPath));

    // Check if file exists
    try {
      if (!require('fs').existsSync(this.scriptPath)) {
        console.error(`Python script not found at ${this.scriptPath}`);
        return { success: false, error: `Python script not found at ${this.scriptPath}` };
      } else {
        console.log(`Found script at ${this.scriptPath}`);
      }
    } catch (error) {
    ...
    try {
      ...
      const adapterPath = path.join(__dirname, '../utils/pythonAdapter.py');

      // Check if adapter exists
      if (!fs.existsSync(adapterPath)) {
        const error = new Error(`Python adapter script not found at ${adapterPath}`);
        console.error('Error starting Python process:', error);
      ...
      console.log(`Using Python adapter at: ${adapterPath}`);

      // Launch the Python process with the adapter
      const args = [adapterPath, this.scriptPath];
  ...
  async getStatus() {
    if (!this.isRunning) {
      console.log('getStatus: Process is not running');
      return { isRunning: false };
    }

    try {
      console.log('getStatus: Process is running with currentProcessInfo:',
        this.currentProcessInfo ? this.currentProcessInfo._id : 'undefined');

      // Get updated process info from database
      const processInfo = await Process.findById(this.currentProcessInfo._id);

      console.log('getStatus: Found processInfo in database:',
        processInfo ? processInfo._id : 'not found');
